...
public class RuntimeDiagnostics
{
        ...
        string[] types = null, int[] sizes = null, string what = null, string operation = null, string tag = null, bool printToConsole = true, bool forceGC = true)
    {
        ...
        foreach (var type in types)
        {
            foreach (var t in sizes)
            {
                ...
                var m = MeasureRuntimeOperation(funMap[type], t, type, operation, what, tag);
                measures.Add(m);
                if (printToConsole)
    ...
    public double MeasureRuntime(Action<REngine, int, Stopwatch> fun, int n)
    {
        ...
        var ticksPerSec = Stopwatch.Frequency;
    ...
    {
        var d = createIntegerArray(n, 256);
    ...
    {
        var dim = (int)Math.Floor(Math.Sqrt(n));
        return dim;
    }
