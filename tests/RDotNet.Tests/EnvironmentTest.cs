...
namespace RDotNet
{
    public class EnvironmentTest : RDotNetTestFixture
    {
        ...
        public void TestCreateEnvironment()
        {
            ...
            Assert.Equal(SymbolicExpressionType.Environment, newEnv.Type);
        ...
        public void TestSetSymbols()
        {
            ...
            Assert.Empty(engine.GlobalEnvironment.GetSymbolNames());
            Assert.Equal(4, env.GetSymbolNames().Length);
            env.SetSymbol("b", engine.CreateIntegerVector(new[] { 6, 7 }));
            env.SetSymbol("c", engine.CreateNumericVector(new[] { Math.PI, Math.E }));
            Assert.Empty(engine.GlobalEnvironment.GetSymbolNames());
            ...
            engine.SetSymbol("b", engine.CreateIntegerVector(new[] { 11 }));
            Assert.Equal(1, engine.GetSymbol("b").AsInteger().Length);
            Assert.Equal(1, engine.GlobalEnvironment.GetSymbol("b").AsInteger().Length);
            Assert.Equal(2, env.GetSymbol("b").AsInteger().Length);
            Assert.Single(engine.GlobalEnvironment.GetSymbolNames());

            engine.SetSymbol("d", engine.CreateIntegerVector(new[] { 1, 2, 3, 4 }));
